...
class Location(db.Model):
    ...
    LocationID = db.Column(db.BigInteger, primary_key=True)
...
class Trip(db.Model):
    ...
    RatecodeID = db.Column(
        db.BigInteger, db.ForeignKey('rate_codes.RatecodeID'))
    store_and_fwd_flag = db.Column(db.String(1))
    PULocationID = db.Column(
        db.BigInteger, db.ForeignKey('locations.LocationID'))
    DOLocationID = db.Column(
        db.BigInteger, db.ForeignKey('locations.LocationID'))
    payment_type = db.Column(
        db.BigInteger, db.ForeignKey('payment_types.payment_type'))
    fare_amount = db.Column(db.Numeric(10, 2))
    extra = db.Column(db.Numeric(10, 2))
    ...
    pickup_loc = db.relationship('Location', foreign_keys=[
                                  PULocationID], backref='trips_starting_here')
    dropoff_loc = db.relationship('Location', foreign_keys=[
                                  DOLocationID], backref='trips_ending_here')
    vendor_ref = db.relationship('Vendors', foreign_keys=[
        VendorID], backref='trips')
